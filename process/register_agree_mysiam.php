...
class RequestsHandler extends DBHandler
{
    ...
    private function invalidateAllRequests()
    {
        ...
        $sql = 'SELECT id_transaction FROM requests_pending WHERE `status`=2 AND ' . $sqlConditions . ';';
        $stmt = ($this->dbh)->prepare($sql);
        $stmt->execute();
    ...
    private function updateUnderRequest($sqlConditions)
    {
        ...
        $this->sqlForNumLangs = "SELECT date_shift, date_shift, shift, id_user, id_shift FROM shifts_assigned WHERE $sqlConditions ;";
    ...
    public function executeTransaction()
    {
        ...
        $sql = 'SELECT id_shift FROM shifts_assigned WHERE under_request=1 ;';
        $this->querySql($sql);
    ...
    public function process()
    {
        ...
        $this->executeSql("LOCK TABLE shifts_assigned WRITE requests_pending WRITE;");
        $this->execute();
