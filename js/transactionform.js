class FormHandler {
    ...
    checkConfirmable(event) {
        ...
        this._$formItems.each(function (idxFormItem) {
            ...
            var currentArrayNumLangs = handler._transaction_form_handler.arrayDateObjects[`${currentDate.getFullYear()}-${currentDate.getMonth() + 1}-${currentDate.getDate()}`].arrayNumLangsByPart[currentShiftPart];
        ...
        var enoughNumLangs = handler.checkNumLangs();
        if (!enoughNumLangs){
            _confirmable = false;
        }
        if (!_confirmable) {
    ...
    checkNumLangs() {
        for (var date in handler._transaction_form_handler.arrayDateObjects) {
            var dateObject = handler._transaction_form_handler.arrayDateObjects[date]
            for (var part in dateObject.arrayNumLangsByPart) {
                ...
                for (var lang in arrayNumLangs) {
                    if (arrayNumLangs[lang] !== null && handler._transaction_form_handler.arrayDateObjects[date].arrayNumLangs[part][lang] < arrayNumLangs[lang]) {
