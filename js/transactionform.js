class FormHandler {
    ...
    checkConfirmable(event) {
        ...
        this._$iOverlap.addClass('invisible');
        ...
        this._$formItems.each(function (idxFormItem) {
            ...
            $.each($selectsInFormItem, function (idxSelect) {
                ...
                console.log($(this).children(`option[value="${this.value}"]`).hasClass('option-warning'));
                ...
                if (idxSelect == 4 && $(this).children(`option[value="${this.value}"]`).hasClass('option-warning')) {
                    ...
                    $.each(handler._$formItems, function (index) {
                        ...
                        if ($selects[1].value === $selectsInFormItem[1].value
                            ...
                            overlap = true;
                            console.log('overlap!', $selects[4].value, $selectsInFormItem[4].value);
                            return false
                        }
                    });
                    if (!found){
                    ...
                    if (overlap){
                        handler._$iOverlap.attr('title', `${$(this).children(`option[value="${this.value}"]`).html()}に２つ以上のかぶるシフトを与えています`).removeClass('invisible');
                    }
                    if (!found || overlap) {
                        error = true;
                        return false
...
formHandler = new FormHandler(arrayShiftsByIdUser);
