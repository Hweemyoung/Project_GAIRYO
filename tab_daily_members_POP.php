<?php
class DailyMembersHandler extends Overloading
{
    private $Y;
    private $currentPage;
    private $dayStart;
    ...
    public function __construct($Y, $currentPage, $dayStart, $dayEnd)
    {
        $this->Y = $Y;
    ...
    {
        $this->dateStart = $this->getDateTimeOfDayFromWeek($this->Y, $this->currentPage, $this->dayStart);
        $this->dateEnd = $this->getDateTimeOfDayFromWeek($this->Y, $this->currentPage, $this->dayEnd);
    }
}

class DateObject extends Overloading
{
    public $arrayLangs = [];
    function __construct($arrayShiftObjects)
    {
        $this->setArrayLangs($arrayShiftObjects);
    }
    public function setArrayLangs($arrayShiftObjects)
    {
        foreach ($arrayShiftObjects as $shiftObject) {
            foreach ($shiftObject->memberObj->arrayLangs() as $lang) {
                if (isset($this->arrayLangs[$lang])) {
                    $this->arrayLangs[$lang]++;
                } else {
                    $this->arrayLangs[$lang] = 1;
                }
            }
        }
    }
...
{
    public $memberObject;
    function __construct($arrayMemberObjectsByIdUser)
    {
